...
```{r}
constituencyChoice <- votingData$constituencyName[141]
...
leaflet(data = simplifiedConstituencyShapefile) %>% 
  ...
  addPolygons(fill = TRUE,
    fillColor = ~pal(shortfallVotesPpnVoted), stroke = TRUE, 
    color = ~ifelse(polygonStroke, "black", "white"), 
    dashArray = ~ifelse(polygonStroke, "", "3"),
    weight = ~ifelse(polygonStroke, 7, 1), fillOpacity = 0.5, 
    label = labels,
    highlight = highlightOptions(
    bringToFront = TRUE)
  ) %>%
  addLegend(pal = pal, values = ~shortfallVotesPpnVoted, 
    labFormat = labelFormat(suffix = "%", transform = function(x) 100 * x),
    opacity = 0.6, title = "Shortfall", position = "bottomright")
```
