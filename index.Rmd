...
click_tract <- eventReactive(input$constituencyMap_shape_click, {
...
observe({
  ...
  map <- leafletProxy('constituencyMap') %>%
      removeShape('htract') %>%
      addPolygons(
        data = simplifiedConstituencyShapefile[simplifiedConstituencyShapefile$GEOID == click_tract(), ], 
        fill = FALSE, color = '#FF00FF', opacity = 1, layerId = 'htract')
  })
# tract_data <- reactive({
#   
