...
The [Marginal constituencies in region](#section-marginal-constituencies-in-region) tab highlights the top marginal contituencies in the selected region.
...
output$constituencyMap <- renderLeaflet({
  ...
  leaflet(data = simplifiedConstituencyShapefile) %>% 
    ...
    addPolygons(
      ...
      fillOpacity = 0.6, 
      layerId = ~POLYGON_ID,
      stroke = TRUE, color = "white", #~ifelse(polygonStroke, "black", "white"), 
      dashArray = 3, #~ifelse(polygonStroke, "", "3"),
      weight = 1, #~ifelse(polygonStroke, 7, 1), 
      label = labels,
...
observe({
  req(click_constituency()) # do this if click_tract() is not null
  ...
  map <- 
    ...
    addPolygons(
      data = simplifiedConstituencyShapefile[simplifiedConstituencyShapefile$POLYGON_ID == click_constituency(), ], 
      fill = FALSE, color = "black", 
      opacity = 1, layerId = 'selectedConstituency')
  })
constituency_data <- reactive({
    # Fetch data for the clicked constituency
  (simplifiedConstituencyShapefile[
    simplifiedConstituencyShapefile$POLYGON_ID == click_constituency(), ]
  )$constituencyName
})
# updateSelectInput(
#   getDefaultReactiveDomain(), "constituencyChoice")
